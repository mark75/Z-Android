...
package com.zandroid.network.lib.websocket;
...
public class WebSocketWriter extends Handler {
	...
	public WebSocketWriter(Looper looper, Handler master, SocketChannel socket,
		...
		mBuffer = new ByteBufferOutputStream(
				options.getMaxFramePayloadSize() + 14, 4 * 64 * 1024);
	...
	private void sendBinaryMessage(WebSocketMessage.BinaryMessage message)
		...
		if (message.mPayload.length > mOptions.getMaxMessagePayloadSize()) {
			throw new WebSocketException(
					"message payload exceeds payload limit");
		}
	...
	protected void sendFrame(int opcode, boolean fin, byte[] payload,
		...
		if (len <= 125) {
			...
			mBuffer.write(new byte[] { (byte) ((len >> 56) & 0xff),
					(byte) ((len >> 48) & 0xff), (byte) ((len >> 40) & 0xff),
					(byte) ((len >> 32) & 0xff), (byte) ((len >> 24) & 0xff),
					(byte) ((len >> 16) & 0xff), (byte) ((len >> 8) & 0xff),
					(byte) (len & 0xff) });
		}
	 ...
	 */
	protected void processMessage(Object msg) throws IOException,
			WebSocketException {
	...
	protected void processAppMessage(Object msg) throws WebSocketException,
		...
		throw new WebSocketException(
				"unknown message received by WebSocketWriter");
	}
}
